...
export const options = {
  scenarios: {
    ...
    concurrent_300: {
      ...
      startTime: '4m', // starts after baseline finishes
      tags: { scenario: 'concurrent_300' },
    },
  },
  thresholds: {
    // Req 3: p95 response time < 1 second
    'http_req_duration{scenario:baseline}': ['p(95)<1000'],
    'http_req_duration{scenario:concurrent_300}': ['p(95)<1000'],
...
export function setup() {
  ...
  const sensorEUIs = sensorsRes.status === 200
    ? (sensorsRes.json() || []).map((s) => s.device_eui).filter(Boolean)
    : [];
...
export default function ({ token, sensorEUIs }) {
  ...
  group('production_view', () => {
    ...
    const ordersRes = http.get(`${BASE_URL}/api/v1/erp/orders`, { headers });
